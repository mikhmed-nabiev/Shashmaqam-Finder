...
std::vector<std::pair<std::string, int>> GenerateHashes(
    ...
    // Sort peaks by time if PeakSort is enabled (enabling may improve performance but accuracy will be damaged)
    if (PeakSortEn) {
    ...
    for (size_t i = 0; i < peaks.size(); i++) {
        for (size_t j = i + 1; j < i + fan_value && j < peaks.size(); j++) {
            ...
            if (time_delta > MIN_HASH_TIME_DELTA && time_delta < MAX_HASH_TIME_DELTA) {
                std::string input = std::to_string(freq1) + "|" + std::to_string(freq2) + "|" + std::to_string(time_delta);
...
// TODO: Optimization
std::vector<std::pair<int, int>> GetPeaks(const std::vector<std::vector<double>>& spectrogram, int radius = 10) {
    std::vector<std::pair<int, int>> peaks;
    std::vector<std::vector<double>> spectrogram_copy(spectrogram.size(), std::vector<double>(spectrogram[0].size()));
